...
Appearance now uses a single ordered flow: Display mode → Workspace colour → Button colour. Six modes (System, Light, Dark, Dim, Midnight black, Sepia) combine with eight workspace palettes. Custom button colours select readable black/white text automatically and save to the account. Profiles have unique editable @usernames and a sidebar subtitle preference (username, bio, job title or account role). Choosing a role label never grants permissions. Existing users receive unique handles through an additive migration.
